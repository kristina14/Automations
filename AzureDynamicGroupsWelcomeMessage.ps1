...
function Get-AzureADSuperCreds(){
    $credentials = Get-AutomationPSCredential -Name "Sharepoint"
    return $credentials;
}
function Get-AzureADNormalCreds(){
    $credentials = Get-AutomationPSCredential -Name "Outlook365"
    return $credentials;
}
...
function Get-DynamicGroupEnvGroupMembers($groupId){
    return (Get-AzureADGroupMember -ObjectId $groupId -All $true) | Select-Object @{n="technion_devops_group_welcome_message";e={$_.ExtensionProperty.extension_6f9a371defc34621b3e6976c5bfdc5f6_technion_devops_group_welcome_message}}, @{n="email";e={$_.UserPrincipalName}} 
}
function Send-DynamicGroupEnvWelcomeMessage($group, $groupId, $to){

    if((Get-DynamicGroupEnvIsUserGotMessage $to $groupId) -eq 0){
        $memberObj = Get-AzureADUser -ObjectId $to.email;
        ...
        if($to.email -eq "stossel@technion.ac.il" -or (-not $global:testing)){
            # -or $to -eq "ymoti@technion.ac.il"
            #Write-Error ("trying to send an email to {0}" -f ($to.email));
            if(($group.MessageReturnEmail).Length -eq 0){
                Send-MailMessage -From "DoNotReply@technion.ac.il" -To $to.email -BodyAsHtml "$generalEmailTemplet" -Subject "הודעת צירוף לקבוצה אוטומטית"  -Credential (Get-AzureADNormalCreds)  -smtpserver smtp.office365.com -Port 587 -UseSsl -Encoding UTF8 
            }
            else{
                Send-MailMessage2 -From "DoNotReply@technion.ac.il" -To $to.email -BodyAsHtml "$generalEmailTemplet" -Subject "הודעת צירוף לקבוצה אוטומטית"  -Credential (Get-AzureADNormalCreds)  -smtpserver smtp.office365.com -Port 587 -UseSsl -ReplyTo $group.MessageReturnEmail
            }
        }
    }
}
...
function Start-DynamicGroupEnvEmailSend(){
    ...
    foreach($group in $groups){
        $foundGroup = (Get-AzureADGroup -SearchString $group.AzureGroup);
        if($null -ne $foundGroup){
            $groupId = $foundGroup.ObjectId;
            if($groupId.GetType().Name -eq "String"){
                $members = Get-DynamicGroupEnvGroupMembers $groupId
                if($group.FirstProcess -eq 0 -or $group.FirstProcess -eq "0"){
                    foreach($member in $members){
                        write-host $member.email;
                        Send-DynamicGroupEnvWelcomeMessage $group $groupId $member;
                    }
                }
                else{
                    foreach($member in $members){
                        Get-DynamicGroupEnvIsUserGotMessage $member $groupId
                    }
                } 
                (Set-DynamicGroupEnvFirstProcessStatus $group 0) | Out-Null
            }
            else{
                foreach($currentGroup in $foundGroup){
...
function Get-DynamicGroupEnvIsUserGotMessage($user, $groupId){

    $technion_devops_group_welcome_message = $user.technion_devops_group_welcome_message;
    if($technion_devops_group_welcome_message.Length -eq 0){
        Set-DynamicGroupEnvIsUserGotMessage $user.email $groupId $technion_devops_group_welcome_message;
        return 0;
    }
    else{
